...
def flipboard(board): # turns every X to an O and every O to an X
	for r in range(0,8):
		for c in range(0,8):
			if board[r][c]=="B": # use double equal for comparison
				board[r][c]="W"	 # use single equal for assignment
			elif board[r][c]=="W":
				board[r][c]="B"
...
def lookahead(board,level,lastmove): # returns the best value for any possible move
	if level==0:
		...
		movelist = legalmoves(board,"W")
		bestval = -998
		for mymove in movelist:
			tempBoard = copy.deepcopy(board)
			flipboard(tempBoard)
			makemove(tempBoard,"W",mymove)
...
theBoard=[[],[],[],[],[],[],[],[]]
clearboard(theBoard)
theBoard[4][4]=theBoard[3][3]="W"
theBoard[3][4]=theBoard[4][3]="B"

gameover = False
who = "W"
where = []
playerDictionary = {"W":"random","B":"random"}
printboard(theBoard)
print(legalmoves(theBoard,"W"))
while not gameover:
